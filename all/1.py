...
response0 = requests.post('https://gigachat.devices.sberbank.ru/api/v1/chat/completions', headers=headers,
                          json=json_data, verify=False)
print(response0.json())
time.sleep(270)
response = requests.get('https://gigachat.devices.sberbank.ru/api/v1/chat/completions', headers=headers, json=json_data,
                        verify=False)
data = response.json()
# #data = json.loads(response.json())
print(data)
time.sleep(270)
response2 = requests.get('https://gigachat.devices.sberbank.ru/api/v1/chat/completions', headers=headers,
                         json=json_data, verify=False)
...
#
# # Использование регулярного выражения для извлечения содержимого src тега img
# match = re.search(r'<img src=\\"([^\\"]+)\\" func=\\"true\\"/>', content)
# if match:
#     img_src = match.group(0)  # Возвращает весь тег <img> с атрибутами
#     print(img_src)
# else:
#     print("Тег <img> не найден.")
#
